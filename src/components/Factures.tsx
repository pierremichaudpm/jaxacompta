...
import { Receipt, Plus, Download, Check, Trash2, FileText, DollarSign, Clock, AlertTriangle } from 'lucide-react';
import { api } from '@/lib/api';
import { generateFacturePDF } from '@/lib/generateFacturePDF';
import type { Transaction, Contact, Projet, CompteBancaire, LigneFacture } from '@/types';

const fmt = (n: number) =>
  new Intl.NumberFormat('fr-CA', { style: 'currency', currency: 'CAD' }).format(n);

const STATUT_OPTIONS = ['Envoyée', 'Payée', 'En retard', 'En attente', 'À valider'];
...
export default function Factures() {
  ...
  const [summary, setSummary] = useState({ total: 0, payees: 0, en_attente: 0, en_retard: 0, montant_impaye: 0 });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const data = await api.get<{ rows: Transaction[]; summary: typeof summary }>(
        `/api/factures?${params}`
      );
      setRows(data.rows);
  ...
  const addLigne = () => {
    setFormLignes(prev => [...prev, { description: '', unite: '', cout_unitaire: '', isHeader: false }]);
  };
  ...
  const updateLigne = (idx: number, field: keyof NewLigne, value: string | boolean) => {
    setFormLignes(prev => prev.map((l, i) => i === idx ? { ...l, [field]: value } : l));
  };
  ...
  const computedLignes = (): LigneFacture[] => {
    return formLignes.map(l => ({
      ...
      montant: l.isHeader ? 0 : (parseFloat(l.unite) || 1) * (parseFloat(l.cout_unitaire) || 0),
      isHeader: l.isHeader,
    }));
  };

  const sousTotal = computedLignes().reduce((sum, l) => sum + l.montant, 0);
  const tpsAmount = formTaxable ? (formUseTVH ? Math.round(sousTotal * 0.13 * 100) / 100 : Math.round(sousTotal * 0.05 * 100) / 100) : 0;
  const tvqAmount = formTaxable && !formUseTVH ? Math.round(sousTotal * 0.09975 * 100) / 100 : 0;
  ...
  const handleDownloadPDF = (tx: Transaction) => {
    const lignes: LigneFacture[] = tx.lignes_facture
      ...
      : [{ description: tx.description || tx.numero_facture || 'Services', unite: 1, cout_unitaire: Number(tx.montant_ht) || 0, montant: Number(tx.montant_ht) || Number(tx.total_ttc) }];
    ...
    const doc = generateFacturePDF({
      ...
      lignes,
      sous_total: Number(tx.montant_ht) || Number(tx.total_ttc) - Number(tx.tps) - Number(tx.tvq),
      tps: Number(tx.tps),
      tvq: Number(tx.tvq),
      total_ttc: Number(tx.total_ttc),
      use_tvh: Number(tx.tvq) === 0 && Number(tx.tps) > 0 && Number(tx.tps) > Number(tx.montant_ht) * 0.06,
    });
  ...
  const handleUpdateStatut = async (tx: Transaction, statut: string) => {
    await api.put('/api/factures', {
      ...
      date_paiement: statut === 'Payée' ? new Date().toISOString().split('T')[0] : tx.date_paiement,
  ...
  const fetchNextNumber = async () => {
    ...
    try {
      const data = await api.get<{ suggested: string }>(`/api/factures?next_number=true&prefix=${prefix}`);
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold mt-1 text-green-700">{summary.payees}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold mt-1 text-red-700">{fmt(Number(summary.montant_impaye))}</p>
          </CardContent>
        </Card>
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow key={tx.id}>
                    <TableCell className="font-mono text-sm">{tx.numero_facture || '—'}</TableCell>
                    <TableCell className="whitespace-nowrap">{tx.date_transaction?.split('T')[0]}</TableCell>
                    <TableCell>{tx.contact_nom || '—'}</TableCell>
                    <TableCell>{tx.projet_code && <Badge variant="outline">{tx.projet_code}</Badge>}</TableCell>
                    <TableCell>
                      {tx.statut_facture ? (
                        <Badge variant={STATUT_COLORS[tx.statut_facture] as 'default' | 'secondary' | 'destructive' | 'outline' || 'outline'}>
                          {tx.statut_facture}
                        </Badge>
                      ) : '—'}
                    </TableCell>
                    <TableCell className="text-right font-mono text-green-700">{fmt(Number(tx.total_ttc))}</TableCell>
                    <TableCell>
                      <div className="flex gap-1">
                        {tx.numero_facture && (
                          <Button variant="ghost" size="icon" className="h-7 w-7" title="Télécharger PDF" onClick={() => handleDownloadPDF(tx)}>
                            <Download className="h-3 w-3" />
                          </Button>
                        )}
                        {tx.statut_facture !== 'Payée' && (
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-green-600" title="Marquer payée"
                            onClick={() => handleUpdateStatut(tx, 'Payée')}>
                            <DollarSign className="h-3 w-3" />
                          </Button>
                        )}
                        {tx.statut_facture === 'Envoyée' && (
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-red-600" title="Marquer en retard"
                            onClick={() => handleUpdateStatut(tx, 'En retard')}>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="space-y-2">
                ...
                <Select value={formClientId} onValueChange={setFormClientId}>
                  ...
                  <SelectContent>
                    {contacts.map(c => <SelectItem key={c.id} value={c.id.toString()}>{c.nom}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label>Projet</Label>
                <Select value={formProjetId} onValueChange={(v) => { setFormProjetId(v); }}>
                  <SelectTrigger><SelectValue placeholder="Choisir..." /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-1">
                  <Input value={formNumero} onChange={e => setFormNumero(e.target.value)} placeholder="CTV_0025-..." />
                  <Button variant="outline" size="icon" className="shrink-0" onClick={fetchNextNumber} title="Auto-générer">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="flex items-center gap-2 pt-6">
                <Checkbox id="taxable" checked={formTaxable} onCheckedChange={v => setFormTaxable(v as boolean)} />
                <Label htmlFor="taxable">Taxable</Label>
              </div>
              <div className="flex items-center gap-2 pt-6">
                <Checkbox id="tvh" checked={formUseTVH} onCheckedChange={v => setFormUseTVH(v as boolean)} disabled={!formTaxable} />
                ...
                <Table>
                  ...
                  <TableBody>
                    {formLignes.map((l, idx) => {
                      const montant = l.isHeader ? 0 : (parseFloat(l.unite) || 1) * (parseFloat(l.cout_unitaire) || 0);
                      return (
                        <TableRow key={idx}>
                          <TableCell>
                            <Checkbox checked={l.isHeader} onCheckedChange={v => updateLigne(idx, 'isHeader', v as boolean)} />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={l.description}
                              onChange={e => updateLigne(idx, 'description', e.target.value)}
                              placeholder={l.isHeader ? "Titre de section..." : "Description du service..."}
                              className={l.isHeader ? 'font-bold' : ''}
                            />
                          </TableCell>
                          <TableCell>
                            {!l.isHeader && (
                              <Input type="number" step="0.25" value={l.unite} onChange={e => updateLigne(idx, 'unite', e.target.value)} placeholder="1" />
                            )}
                          </TableCell>
                          <TableCell>
                            {!l.isHeader && (
                              <Input type="number" step="0.01" value={l.cout_unitaire} onChange={e => updateLigne(idx, 'cout_unitaire', e.target.value)} placeholder="0.00" />
                            )}
                          </TableCell>
                          ...
                          <TableCell>
                            {formLignes.length > 1 && (
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500" onClick={() => removeLigne(idx)}>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleCreate} disabled={saving || !formClientId || !formNumero || sousTotal === 0}>
